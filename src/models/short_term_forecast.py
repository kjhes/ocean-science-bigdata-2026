"""환경변수(기온·풍속·강수량)를 반영한 단기(1일 뒤) 수온 예측 모델.

1차 멘토링 후속: "수온 하나만으로는 정확도가 나오지 않는다. 기온·일사량·
강수량·해풍을 추가하라"는 지적에 대응. 강수량은 결측=무강수(0mm) 확정 후
포함(효과는 미미), 일사량은 지역별 관측시작 시점이 달라 4지역 공통 변수로
쓸 수 없어 최종 보류(검증 상세: docs/단기예측_환경변수_검증.md 4-2·4-3절).
